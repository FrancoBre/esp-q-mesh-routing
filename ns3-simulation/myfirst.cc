...
* ONGOING
*
* copy and paste this code in a fresh ns-3 installation, under scratch/myfirst.cc, build and run
* /ns3 build && ./ns3 run scratch/myfirst
*
* as you may read in the comments I'm fucking dumb and this code is crap but I'm trying to make
* it work, gotta improve it when that happens
*/
...
/**
 * Forward declaration of Mesh Test application class because I'm fucking dumb and
 * I don't know how to use CMakeLists and how to link the mf header files
 */
...
/**
 * Forward declaration of Q learning application class because I'm fucking dumb and
 * I don't know how to use CMakeLists and how to link the mf header files
 *
 */
...
public:
    ...
    void Setup(Ptr<Node> node, Ipv4Address sinkAddress, uint16_t port, double interval, uint32_t packetSize, MeshTest* meshTest);
...
void QLearningApplication::Setup(Ptr<Node> node, Ipv4Address sinkAddress, uint16_t port, double interval, uint32_t packetSize, MeshTest* meshTest)
{
    m_node = node;
...
{
    // NS_LOG_INFO("QLearningApplication::StartApplication()");
    m_socket = Socket::CreateSocket(m_node, TypeId::LookupByName("ns3::UdpSocketFactory"));
    m_socket->Bind();
    m_socket->Connect(InetSocketAddress(m_sinkAddress, m_port));
    m_socket->SetRecvCallback(MakeCallback(&QLearningApplication::ReceivePacket, this));
    ScheduleNextTx();
}
...
{
    m_sendEvent = Simulator::Schedule(Seconds(m_interval), &QLearningApplication::SendPacket, this);
}
...
void QLearningApplication::ReceivePacket(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        ...
        std::cerr << __func__ << "() -- Node" << currentNodeId << " received mf packet from: " << from << std::endl;
        ...
        if (currentNodeId == m_meshTest->GetMasterNodeId())
        {
            ...
            std::cerr << __func__ << "() -- Found master node! Increasing reward by +100" << std::endl;
            m_meshTest->UpdateQTable(currentNodeId, nextHop, 100.0);
            // TODO acá finaliza el episodio, hay que hacer que termine acá la ejecución
            //  en vez de que sea por tiempo, y que haya un contador que vaya bajando los episodios
        }
        else
        {
            // Non-master node: provide a negative reward
            std::cerr << __func__ << "() -- Non master node, decreasing reward by -1" << std::endl;
            m_meshTest->UpdateQTable(currentNodeId, nextHop, -1.0);
        }

        // Forward the packet to the next hop
        std::cerr << __func__ << "() -- Sending packet to: " << nextHop << std::endl;
        m_socket->Connect(InetSocketAddress(m_meshTest->interfaces.GetAddress(nextHop), m_port));
        m_socket->Send(packet);
    }
}
...
uint32_t MeshTest::GetMasterNodeId() const
{
...
void MeshTest::CreateNodes()
{
    std::cerr << std::endl;
    // NS_LOG_DEBUG("Sent " << p->GetSize() << " bytes");
    std::cerr << "Creating mf nodes: y_size: " << m_ySize << ", x_size: " << m_xSize << std::endl;
    nodes.Create(m_ySize * m_xSize);
    m_masterNodeId = nodes.Get(m_ySize * m_xSize - 1)->GetId(); // Set the last node as the master node
    std::cerr << "Mf master node id is " << m_masterNodeId << std::endl;
...
void MeshTest::InstallApplication()
{
    ...
    for (uint32_t i = 0; i < nodes.GetN(); ++i)
    {
        if (i != sinkNodeId)
        {
            ...
            app->Setup(nodes.Get(i), interfaces.GetAddress(sinkNodeId), portNumber, m_packetInterval, m_packetSize, this);
...
// Function to print the qTable
std::ostream& operator<<(std::ostream& os, const std::map<uint32_t, std::map<uint32_t, double>>& qTable) {
    ...
    for (auto i = nodes.Begin(); i != nodes.End(); ++i)
    {
        ...
        for (auto j = nodes.Begin(); j != nodes.End(); ++j)
        {
            if (i != j)
            {
                ...
                qTable[nodeId][neighborId] = 0.0; // Initialize Q-value
            }
        }
    }
...
void MeshTest::UpdateQTable(uint32_t stateFrom, uint32_t stateTo, double reward)
{
    std::cerr << std::endl;
    ...
    qTable[stateFrom][stateTo] = currentQ + m_alpha * (reward + m_gamma * maxQ - currentQ);

    // Log the updated Q-Table
    std::cerr << __func__ << "() -- Updated Q-Table for state " << stateFrom << " to " << stateTo << ": " << qTable[stateFrom][stateTo] << std::endl;
    std::cerr << __func__ << "() -- Q table is:" << qTable << std::endl;
}
...
uint32_t MeshTest::ChooseAction(uint32_t state)
{
    ...
    if ((double)rand() / RAND_MAX < m_epsilon)
    {
        ...
        std::cerr << __func__ << "() -- Exploring: chosen action " << it->first << " for state " << state << std::endl;
        return it->first;
    }
    else
    {
        ...
        std::cerr << __func__ << "() -- Exploiting: chosen action " << bestAction << " for state " << state << std::endl;
        return bestAction;
    }
}
